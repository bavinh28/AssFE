...
const Menmain = ({ post: { id, name, price, image }, deletePost }) => {
    return (
        <>
            {/* <div className="box">
            <a href="https://youtu.be/s6zR2T9vn2c" className="image fit"><img src={image} alt="" /></a>
            <div className="inner">
                <h3>{name}</h3>
                <p>{price}</p>
                <button className="button fit" onClick={(e) => deletePost(id)}>Delete</button>
            </div>
        </div> */}
            <div className="w-full max-w-sm mx-auto rounded-md shadow-md overflow-hidden my-2  ">
            <Link to={`/posts/${id}`}>
